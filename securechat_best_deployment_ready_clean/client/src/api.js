...
export const resolveFileUrl=value=>{if(!value)return'';if(/^(https?:|data:|blob:)/i.test(value))return value;const token=getToken();return API_URL+value+(token?'?token='+encodeURIComponent(token):'')};
